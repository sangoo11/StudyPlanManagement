...
class CourseService {
  static createCourse = async (
    subjectID,
    { courseCode, semester, year, teacherID }
  ) => {
    // Validation
    ...
    if (subjectCode !== currentSubject.subjectCode)
      throw new Error("Subject code does not match");
  ...
  static editCourse = async (
    courseID,
    { semester, year, teacherID, active }
  ) => {
    // Validation
    ...
    // TeacherID
    if (teacherID) {
      if (currentCourse.active === false && !active)
  ...
  static getStudentCourse = async (courseID) => {
    ...
    if (!enrollmentList)
      throw new Error("This course does not have any student");
