...
function AddAward({ onClose }) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.post("http://localhost:8080/v1/api/award-type/", formData);
            alert("Thêm giải thưởng thành công!");
            onClose();
        } catch (err) {
            setError("Không thể thêm giải thưởng. Vui lòng kiểm tra lại!");
        }
    };
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50">
            <div className="flex flex-col w-[50vw] bg-gray-200 p-6 rounded">
                <h2 className="text-3xl font-bold text-center mb-4">Thêm giải thưởng</h2>
